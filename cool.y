  ...
  #define cool_yylloc curr_lineno  /* use the curr_lineno from the lexer for the location of tokens */
    ...
    */
    ...
    class : CLASS TYPEID '{' features '}' ';'
          { @$ = @6; $$ = class_($2,idtable.add_string("Object"),$4, stringtable.add_string(curr_filename)); }
    ...
    feature : method_declaration ';' {  @$ = @2; $$ = $1; }
            | attribute_declaration ';' { @$ = @2; $$ = $1; }
            | error ';' {  }
    ;
    ...
    attribute_declaration :  OBJECTID ':' TYPEID { @$ = @3; $$ = attr($1, $3, no_expr()); }
                          | OBJECTID ':' TYPEID ASSIGN '{' expression '}' { @$ = @7; $$ = attr($1, $3, $6); }
                          | OBJECTID ':' TYPEID ASSIGN expression { @$ = @5; $$ = attr($1, $3, $5); }
    ;

    formals : { $$ = nil_Formals(); }
            | formal { @$ = @1; $$ = single_Formals($1); }
            | formals ',' formal  { @$ = @2;  $$ = append_Formals($1,single_Formals($3)); }
    ;

    formal  :  OBJECTID ':' TYPEID  { @$ = @3; $$ = formal($1, $3); }
    ;

    expression  :  OBJECTID ASSIGN expression  { @$ = @3; $$ = assign($1, $3); }
                | dispatch_expression  { @$ = @1; $$ = $1; }
                | LET let_expression  { @$ = @2; $$ = $2; }
                | IF expression THEN expression { @$ = @4; $$ = cond($2,$4, no_expr()); }
                | IF expression THEN expression ELSE expression FI { @$ = @4; $$ = cond($2,$4,$6); }
                | WHILE expression LOOP expression POOL { @$ = @5; $$ = loop($2, $4); }
                | '{' block_expressions '}' { @$ = @2; $$ = block($2); }
                | CASE expression OF branches ESAC { @$ = @5; $$ = typcase($2, $4); }
                | NEW TYPEID  { @$ = @2; $$ = new_($2); }
                | ISVOID expression  { @$ = @2; $$ = isvoid($2); }
                | expression '+' expression { @$ = @3; $$ = plus($1, $3); }
                | expression '-' expression { @$ = @3; $$ = sub($1, $3); }
                | expression '*' expression { @$ = @3; $$ = mul($1, $3); }
                | expression '/' expression { @$ = @3; $$ = divide($1, $3); }
                | '~' expression { @$ = @2; $$ = neg($2); }
                | NOT expression { @$ = @2; $$ = comp($2); }
                | expression LE expression { @$ = @3; $$ = leq($1, $3); }
                | expression '<' expression { @$ = @3; $$ = lt($1, $3); }
                | expression '=' expression { @$ = @3; $$ = eq($1, $3); }
                | '(' expression ')' { @$ = @2; $$ = $2; }
                | INT_CONST { @$ = @1; $$ = int_const($1); }
                | STR_CONST { @$ = @1; $$ = string_const($1); }
                | BOOL_CONST { @$ = @1; $$ = bool_const($1); }
                | OBJECTID  { @$ = @1; $$ = object($1); }
    ;
